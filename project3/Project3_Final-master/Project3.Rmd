...
For indeed.com, we scraped 110 job listings from the first 11 pages. Each page has 10 job listings.There are over 12,000 data scientist jobs listed on indeed and we stuck to a simple size to give similar weight to all three job sites.The indeed page numbers go like 0, 10, 20,...so we formed a loop that goes with a sequence of 10 to scrape the required data. For this site, we used XPath selector to traverse the html document to llok for specific nodes.
```{r message=FALSE, out.width='70%', fig.show='hold'}
knitr::include_graphics('https://github.com/abhajjam/Project3_Final/blob/master/LinkedIn.png')
```
...
For thi ssite we used the CSS selector to select to traverse the html document and look for the specific nodes.
...
For this site we used the CSS selector to traverse the html document looking for specific nodes, like so:
```{r message=FALSE, out.width='70%', fig.show='hold'}
knitr::include_graphics('https://github.com/abhajjam/Project3_Final/blob/master/Vidhya.png')
```

```{r message=FALSE, out.width='70%', fig.show='hold'}
knitr::include_graphics('https://github.com/abhajjam/Project3_Final/blob/master/VidhyaJobDescription.png')
```
...
As per the requirements of Project, all the fies were supposed to be pushed in MySQL in the database but before doing that, let's just merge all the files together and make a single dataframe for further analysis. Before going to MySQL, let's just bind all three files using rbind function. New schema was created and the new file 'datajobs' was then pushed in MySQL in the database. 
...
After completion of web scraping and saving the file into database and getting back the relevant information, let's do some visualization to see the most frequent words from the data. 
...
After plotting the horizontal barchart on the data, it seems that data is the most frequent word used in the job descriptions which make sense obviously. If we take a look at the plot, it seems that learning, business, team, science, machine, analytics are the most frequently used words in the job description. 




Let's wordCloud this to see it in a different way !! While generating the word cloud, we had to set the minimum and maximum limit to create a better understable picture of the data. The wordCloud shows same result as the barchart. 
...
As we discussed above that the data is not yet clean that's why we have to clean the data and remove the irrelevant stuff. We are only going to add the technical skills that are most frequently used in the data through using mutation and conditional function as shown in the below chunk of codes. 
...
tech_chart <- ggplot(listings_ts, aes(x = reorder(skills, frequency), y= frequency))+
              ...
              labs(title = "Indeed Data Science required tech skills frequency", x = "Tech skills", y = "Frequency")+
...
soft_chart <- ggplot(listings_ss, aes(x = reorder(skills, -frequency), y= frequency))+
              geom_bar(stat = "identity", fill="steelblue")+
              labs(title = "Indeed Data Science required soft skills frequency", x = "Soft skills", y = "Frequency")+
